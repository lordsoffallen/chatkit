...
import type { Chat, Message, VisibilityType } from "@/types/chat";
...
export async function deleteChatById({ id }: { id: string }) {
  try {
    ...
    await db.delete(artifact).where(eq(artifact.chatId, id));
...
export async function deleteAllChatsByUserId({ userId }: { userId: string }) {
  try {
    ...
    await db.delete(artifact).where(inArray(artifact.chatId, chatIds));
